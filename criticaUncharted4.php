<?php
include_once("includes/bodyBase.inc.php");
top();
?>
    <!-- Header End -->

    <!-- Details Hero Section Begin -->
    <section class="details-hero-section set-bg" data-setbg="img/wallpapers/uncharted4.jpg">
        <div class="container">
            <div class="row">

                    <div class="details-hero-text">
                        <div class="label"><span>Análise</span></div>
                        <h1 style="text-shadow: 2px 2px 0px #FF0000;">Uncharted 4: A Thief's End</h1>
                        <ul>
                            <li>by <span>Admin</span></li>
                            <li><i class="fa fa-clock-o"></i> Aug 01, 2019</li>
                            <li><i class="fa fa-comment-o"></i> 2</li>
                        </ul>
                    </div>

            </div>
        </div>
    </section>
    <!-- Details Hero Section End -->

    <!-- Details Post Section Begin -->
    <section class="details-post-section spad">

        <div style="float: right;">
            <h2 style="text-shadow: 2px 2px 0px #FF0000; "><strong>Trailer:</strong></h2>
            <video width="620" height="480" controls>
                <source src="Videos/Uncharted4.mp4" type="video/mp4">

                Your browser does not support the video tag.
            </video>
        </div>


        <div class="container">
            <div class="row">

                    <div class="details-text" >
                        <div style="color: white">
                            <span style="color: #FFFFFF;"><b>Analista:</b></span>
                            <span>Balmiro Pereira  |</span>
                            &nbsp;
                            <span>17/11/2020</span>
                        </div>
                        <br>


                        <div class="dt-desc">
                            <p style="font-size: 16px">A história de Uncharted 4 é estabelecida em um
                                punhado de capítulos convincentes que tecem seu caminho através de diferentes períodos de tempo com um
                                toque cinematográfico fortemente direcionado. Embora sua configuração seja excessivamente familiar -
                                Nathan Drake e Elena Fisher estão tentando se aposentar do heroísmo de ação e viver uma vida normal até que
                                o irmão supostamente morto de Nate apareça com uma oferta que ele não pode recusar - uma forte linha emocional
                                nasce do A luta dos personagens para conciliar suas responsabilidades adultas com a promessa de excitação que
                                secretamente desejam. Uncharted 4 faz um trabalho excelente em explorar um grupo de aventureiros mais cansados do
                                mundo, com suas preocupações e reflexões em camadas ao longo de seus momentos de silêncio.</p>



                            <p style="font-size: 16px">Essas conversas incidentais são uma maravilha.
                                É aqui que vemos personagens eriçados e amolecidos, trazidos lentamente
                                à vida com uma escrita ponderada e um elenco de voz incomparável. Apresentações dos veteranos da série Nolan North (Nathan Drake), Emily Rose (Elena Fisher) e
                                Richard McGonagle (Victor Sullivan) são tão generosos como sempre, enquanto os recém-chegados Troy Baker (Samuel Drake), Laura Bailey (Nadine Ross) e Warren Kole
                                (Rafe Adler) é bem discreto em papéis mais enigmáticos..</p>


                            <p style="font-size: 16px">
                                Os personagens companheiros de Uncharted 4 também nunca quebram o feitiço em seções mais frenéticas ou tensas.
                                Se você escolher jogar furtivamente, eles se agacharão na grama alta ao seu lado (e ao contrário de Ellie em The Last of Us,
                                eles fazem um excelente trabalho em ficar fora das linhas de visão do inimigo). Se eles estiverem no seu caminho durante a escalada,
                                eles permitirão que você passe por cima deles. Eles são competentes em tiroteios, úteis na travessia e normalmente espirituosos. Eles parecem vitais.
                            </p>


                        </div>
                        <div class="dt-overall-rating">
                            <div class="or-heading">
                                <div class="or-item" style="padding-left: 150px">
                                    <div class="or-loader">

                                        <div class="loader-circle-wrap">
                                            <div class="loader-circle">
                                                <span class="circle-progress-2" data-cpid="circle1" data-cpvalue="88"  data-cpcolor="#4bcf13"></span>
                                                <div class="review-point">
                                                    <span style="padding-left: 10px; color: #FFFFFF">User Rating:</span>
                                                    <div style="padding-left: 10px; padding-top: 10px">94</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="or-loader">

                                        <div class="loader-circle-wrap">
                                            <div class="loader-circle">
                                                <span class="circle-progress-2" data-cpid="circle2" data-cpvalue="94"  data-cpcolor="#c20000"></span>
                                                <div class="review-point">
                                                    <span style="padding-left: 10px; color: #FFFFFF">User Rating:</span>
                                                    <div style="padding-left: 10px; padding-top: 10px">94</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="or-skill">
                                    <div class="skill-item">
                                        <p>Design</p>
                                        <div id="bar-1" class="barfiller">
                                            <span class="fill" data-percentage="90"></span>
                                            <div class="tipWrap" style="display: inline;">
                                                <span class="tip"></span>
                                                <span class="bar-point"><b>90</b></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="skill-item">
                                        <p>Graficos</p>
                                        <div id="bar-2" class="barfiller">
                                            <span class="fill" data-percentage="84"></span>
                                            <div class="tipWrap" style="display: inline;">
                                                <span class="tip"></span>
                                                <span class="bar-point"><b>84</b></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="skill-item">
                                        <p>História</p>
                                        <div id="bar-3" class="barfiller">
                                            <span class="fill" data-percentage="94"></span>
                                            <div class="tipWrap" style="display: inline;">
                                                <span class="tip"></span>
                                                <span class="bar-point"><b>94</b></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="skill-item">
                                        <p>Sountrack</p>
                                        <div id="bar-4" class="barfiller">
                                            <span class="fill" data-percentage="85"></span>
                                            <div class="tipWrap" style="display: inline;">
                                                <span class="tip"></span>
                                                <span class="bar-point"><b>85</b></span>
                                            </div>
                                        </div>
                                    </div>

                            </div>
                            <div class="or-qualities">
                                <div class="qualities-item">
                                    <p>O bom:</p>
                                    <hr>
                                    <ul>
                                        <li><i class="fa fa-check"></i> Graficos</li>
                                        <li><i class="fa fa-check"></i> Soundtrack</li>
                                        <li><i class="fa fa-check"></i> História</li>
                                        <li><i class="fa fa-check"></i> Imersão</li>

                                    </ul>
                                </div>
                                <div class="qualities-item bad-item">
                                    <p>O mau:</p>
                                    <hr>
                                    <ul>
                                        <li><i class="fa fa-close"></i> Nao sei</li>

                                    </ul>
                                </div>
                            </div>
                            <div class="or-rating">
                                <p><span>User Rating:</span> 9.0/10 ( 23 votes )</p>
                                <div class="rating-star">
                                    <i class="fa fa-star"></i>
                                    <i class="fa fa-star"></i>
                                    <i class="fa fa-star"></i>
                                    <i class="fa fa-star"></i>
                                    <i class="fa fa-star"></i>
                                </div>
                            </div>
                        </div>
                            <div class="dt-quote">
                                <div style="color: white">
                                    <span><b>Simão Bercial</b></span>
                                    <span style="float: right"><b>17/11/2020</b></span>
                                    <p style="font-size: 30px;margin-top: 15px"><span style="color: lawngreen">90%</span></p>

                                </div>
                                <hr>
                                <br>
                                <p>Embora muito divertido. Parecia que eu já tinha jogado o jogo. A história era boa.
                                    Mas às vezes a jogabilidade me deixava querendo. O websling foi maravilhoso,
                                    mas ir para os marcadores parecia uma rotina. Particularmente com as missões secundárias que
                                    parecem muito decepcionantes, raramente com algum conteúdo interessante envolvido.
                                    Apenas os dois vilões principais pareciam uma ameaça válida, deixando os outros facilmente derrotados,
                                    mesmo estando em maior número do que você.</p>

                                <p>  Eu sinto que houve muito trabalho em algumas áreas,
                                    e não muito em outras, o que me deixou ignorando o conteúdo paralelo depois de um ponto
                                    e apenas navegando pela história principal. Eu gostei do meu tempo com o jogo e ficarei feliz
                                    em ver como a inevitável sequência o melhora, mas não acho que voltarei para limpar a cidade do pós-jogo.</p>
                            </div>

                            <div class="dt-quote">
                                <div style="color: white; width: 100%">
                                    <span><b>Guilherme Ribeiro</b></span> <span style="float: right"><b>17/11/2020</b></span>
                                    <p style="font-size: 30px;margin-top: 15px"><span style="color: lawngreen">100%</span></p>
                                </div>
                                <hr>
                                <br>
                                <p>Já joguei este jogo 5/6 vezes porque é incrível, espetacular e incrível !!.
                                    Este é outro jogo perfeito para mim. Ok, sou um pouco influênciado porque o Homem-Aranha é meu herói favorito,
                                    então quando jogo um jogo excelente como este tendo tendência a ficar muito feliz.</p>
                            </div>
                            <div class="dt-quote">
                                <div style="color: white; width: 100%">
                                    <span><b>Orlando Lopes</b></span> <span style="float: right"><b>17/11/2020</b></span>

                                    <p style="font-size: 30px; margin-top: 15px"><span style="color: red">40%</span></p>
                                </div>
                                <hr>
                                <br>
                                <p>Meh.</p>
                            </div>
                        <div class="dt-tags">
                            &nbsp;
                            &nbsp;
                            <a href="#"><span id="tag1">Gaming</span></a>
                            <a href="#"><span id="tag2">Platform</span></a>
                            <a href="#"><span id="tag3">Playstation</span></a>
                            <a href="#"><span id="tag4">Naughty Dog</span></a>
                            <a href="#"><span id="tag5">uncharted</span></a>
                        </div>
                        <div class="dt-share">
                            <div class="ds-title">Partilha:</div>
                            <div class="ds-links">
                                <button  id="btnLike" onclick="countClicks(this)" class="fa fa-heart text-danger"></button>
                                   &nbsp;
                                   &nbsp;
                                <a href="#"><i class="fa fa-facebook"></i></a>
                                <a href="#"><i class="fa fa-instagram"></i></a>
                            </div>
                        </div>

                            <div class="dt-leave-comment" >

                                <span style="font-size: 30px; color: #FFFFFF"> &nbsp;<strong>Deixa um comentário:</ strong</span>
                                <form action="#" style="padding-top: 20px" >
                                    <div class="input-list" >
                                        <input type="number" placeholder="Rating" style="width: 100px; color: #FFFFFF;">
                                    </div>
                                    <textarea required spellcheck="true"  rows="100" placeholder="Message" style="color: #FFFFFF;  "></textarea>
                                    <button type="submit">Submeter</button>
                                </form>
                            </div>
                    </div>

                </div>
            </div>
        </div>
        </div>
    </section>
    <!-- Details Post Section End -->

    <!-- Footer Section Begin -->
<?php
bottom();
?>